...
export default class SocialMedia extends Component {
  ...
  render() {
    return (
      <div>
        <h2 style={{ paddingBottom: '15px', fontFamily: 'Futura' }}>Your Links </h2>
        {
          (this.state.links) ? 
          (<table className="table table-bordered table-responsive table-hover">
            <tbody> 
              {this.state.links.map((link, key) => {
                return(
                  <tr key={key}>
                    <td colSpan="1"><a href={link.url} target="_blank" style={{ fontSize:'20px' }}><span className="glyphicon glyphicon-folder-open" style={{ fontSize:'20px', padding: '25px'}}></span>{link.name} </a>
                    </td>
                  </tr>) 
                })
              }
        ...
        <p className="App-intro">
          <button onClick={ () => this.handleClick() } style={{ paddingLeft: '40px', paddingRight: '40px', borderRadius: '6px', fontFamily: 'Futura'}}>+</button>
        </p>
